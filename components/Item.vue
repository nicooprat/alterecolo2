<template>
  <li :class="['item', {'is-checked': checked}]">
    ...
    <nuxt-link class="title" :to="`alternative/${item.slug}`">{{item.Alternative}}</nuxt-link>
    <nav class="links">
      ...
      <nuxt-link class="comments" :to="`alternative/${item.slug}`">
        <a :href="$router.resolve({to: `alternative/${item.slug}#disqus_thread`}).route.fullPath"></a>
